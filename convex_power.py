...
def forward2(c, demand, b, alpha, Cb, G, g, ak, gk):
    N = len(G)
    m = len(c)
    n = len(G[0])
    ...
    M1.addConstrs(x[i] == alpha * z[i] + b[i] + gp.quicksum(Cb[i][k] * demand[k] for k in range(len(demand)))
                  for i in range(m))
...
def convexf_sub(x, c, G, g, demand):
    d = len(G[0])  # dimension of primitive sets
    d1 = len(x[0])  # dimension of the forward problem
    d2 = len(G)  # number of the constraints of the primitive set
    d3 = len(demand[0])
    ...
    m.setObjective(1/n*gp.quicksum(gammaf[i][j]**2 for i in range(n) for j in range(d1))
                   + 1/n*gp.quicksum(gammao[i]**2 for i in range(n)),GRB.MINIMIZE)
...
def cL_sub_true(c, d, x, xtrue):  # true sub-optimality loss
    ...
    for i in range(n):
        ...
        res_o[i] = gammao[i].X
        for j in range(m):
            res_f[i][j] = gammaf[i][j].X
    return [res_f, res_o, M1.objVal / n]


def cL_pA(x, c, G, g, dimC, delta, alpha, Cb, b):
    d = len(G[0])  # dimension of primitive sets
    d1 = len(x[0])  # dimension of the forward problem
    ...
    for i in range(n):
        beta[i] = {}
        for j in range(d1):
            beta[i][j] = m.addConstr( x[i][j] - z[i][j]
    ...
    m.addConstrs(
        gp.quicksum(c[i][j] * (x[i][j] + gammao[i][j] - b[j] - gp.quicksum(Cb[j][k] * delta[i][k] for k in range(nL)))
                    for j in range(d1)) + gp.quicksum(lamb[i][j] * g[j] for j in range(d2)) <= 0 for i in range(n))
    ...
    for i in range(n):
        ...
        for j in range(d1):
            res_f[i][j] = gammaf[i][j].X
    return [res_f, m.objVal]


def cL_sA(x, c, G, g, dimC, delta, alpha, Cb, b):
    d = len(G[0])  # dimension of primitive sets
    d1 = len(x[0])  # dimension of the forward problem
    d2 = len(G)  # num of constraints
    n = len(x)  # data num
    ...
    for i in range(n):
        ...
        for j in range(d1):
            beta[i][j] = m.addConstr(
                x[i][j] -  z[i][j] - b[j] - gp.quicksum(Cb[j][k] * delta[i][k] for k in range(nL)) + gammaf[i][j] == 0)
    mu = {}
    for i in range(n):
        mu[i] = {}
        for k in range(d):
...
if __name__ == "__main__":
    ...
    now = time.time()
    [opt_alpha, optval, Post_b, opt_cb] = convexf(opt_g, xiv, G, g, demand)
    #[opt_alpha, optval, Post_b, opt_cb] = convexf_sub(opt_g, xiv, G, g, demand)
    end = time.time()
    ...
    for i in range(test_num):
        [opt_x, optr] = forward2(test_price[i], test_demand[i], Post_b, opt_alpha, opt_cb, G, g, ak, gk)
    ...
    [res_f, res_o, true_subopt] = cL_sub_true(test_price, test_demand, optx, optx_true)
    [res, out_predict] = cL_pA(optx_true, test_price, G, g, 5, test_demand, opt_alpha, opt_cb, Post_b)
    [res_fout, res_oout, out_subopt] = cL_sA(optx_true, test_price, G, g, 5, test_demand, opt_alpha, opt_cb, Post_b)
    print(
