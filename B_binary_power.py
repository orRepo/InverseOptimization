...
def inverse_fixed_binary_nonconvex(signal2, H, h, obj_signal, opt_solution, dp):
    m = gp.Model("main")
    m.setParam('TimeLimit', 3600)
    K = len(signal2[0])
    N = len(signal2)
    d = len(opt_solution[0])
    signal = np.concatenate((np.ones((N, 1)), signal2), axis=1)

    gamma, B, alpha, b, zeta, lamb, obj = {}, {}, {}, {}, {}, {}, {}
    for i in range(N):
        gamma[i], lamb[i], zeta[i] = {}, {}, {}
        obj[i] = m.addVar(lb=0)
        for k in range(K+1):
            lamb[i][k], zeta[i][k] = {}, {}
            for j in range(len(h)):
                lamb[i][k][j] = m.addVar(lb=-GRB.INFINITY, ub=0)
            for j in range(dp):
                zeta[i][k][j] = m.addVar(lb=-GRB.INFINITY)
        for j in range(d):
            gamma[i][j] = m.addVar(lb=-GRB.INFINITY)
    for k in range(K+1):
        B[k], b[k] = {}, {}
        alpha[k] = 1 #m.addVar()
        for i in range(d):
            B[k][i] = {}
            b[k][i] = m.addVar(lb=-GRB.INFINITY)
            for j in range(dp):
                B[k][i][j] = m.addVar(vtype=GRB.BINARY)

    for i in range(N):
        m.addGenConstrNorm(obj[i], [gamma[i][j] for j in range(d)], 2, "normconstr")

    m.setObjective(gp.quicksum(obj[i] for i in range(N)), GRB.MINIMIZE)

    m.addConstrs(opt_solution[i][j]+gamma[i][j] == sum(b[k][j]*signal[i][k] for k in range(K+1))
                 +gp.quicksum(B[k][j][p]*zeta[i][k][p]*signal[i][k] for p in range(dp)
                 for k in range(K+1)) for j in range(d) for i in range(N))

    m.addConstrs(sum(H[j][p]*zeta[i][k][p] for p in range(dp))
                 - alpha[k]*h[j] <= 0 for j in range(len(h)) for k in range(K+1) for i in range(N))

    m.addConstrs(sum(obj_signal[i][j]*(opt_solution[i][j]+gamma[i][j]
                 - sum(b[k][j]*signal[i][k] for k in range(K+1))) for j in range(d))
                 - sum(lamb[i][k][j]*h[j] for j in range(len(h)) for k in range(K+1)) <=0
                 for i in range(N))

    m.addConstrs(signal[i][k]*gp.quicksum(alpha[k]*obj_signal[i][j]*B[k][j][p] for j in range(d))
                 == gp.quicksum(lamb[i][k][j]*H[j][p] for j in range(len(h)))
                 for i in range(N) for k in range(K+1) for p in range(dp))
    #m.setParam("PreQLinearize", 1)
    #m.setParam("NonConvex", 2)
    m.optimize()

    value1 = m.objVal/N
    opt_b, opt_cb = np.zeros(d), np.zeros((d, K))
    A, C = np.zeros((d, dp)), np.zeros((K, d, dp))
    for i in range(d):
        opt_b[i] = b[0][i].X
        for k in range(K):
            opt_cb[i][k] = b[k+1][i].X
        for j in range(dp):
            A[i][j] = B[0][i][j].X*alpha[0].X
            for k in range(K):
                C[k][i][j] = B[k][i][j].X*alpha[k].X

    return [value1, opt_b, opt_cb, A, C]


def inverse_fixed_binary(signal2, H, h, obj_signal, opt_solution, dp):
    m = gp.Model("main")
    m.setParam('TimeLimit', 3600)
    K = len(signal2[0])
    N = len(signal2)
    ...
    m.addConstrs(sum(H[j][p]*zeta[i][k][p] for p in range(dp))
                 - h[j] <= 0 for j in range(len(h)) for k in range(K+1) for i in range(N))
    ...
    m.addConstrs(signal[i][k]*gp.quicksum(obj_signal[i][j]*B[k][j][p] for j in range(d))
                 == gp.quicksum(lamb[i][k][j]*H[j][p] for j in range(len(h)))
                 for i in range(N) for k in range(K+1) for p in range(dp))
    #m.setParam("PreQLinearize", 1)
    #m.setParam("NonConvex", 2)
...
# def inverse_fixed_binary(signal, H, h, obj_signal, opt_solution, dp):
#     m = gp.Model("main")
#     m.setParam('TimeLimit', 3600)
#     K = len(signal[0])
#     N = len(signal)
...
if __name__ == "__main__":
    ...
    if (needdataset):
        ...
        for n in range(datanum):
            ...
            if (fixdeman):
                [buffer2, buffer3] = powersys(xiv[n], fixdemand, noise)
                opt_g.append(buffer2)
                demand.append(fixdemand)
            else:
                [buffer2, buffer3] = powersys(xiv[n], buffer1, noise)
                opt_g.append(buffer2)
    ...
    # main here #########################################################################################
    [value, opt_b, opt_cb, A, C] = inverse_fixed_binary(demand, G, g, xiv, opt_g, dim_p)
    #[opt_beta, opt_mu, opt_z, value, opt_b, opt_cb] = inverse_fixed_binary(A, C, opt_g, xiv, eps1, eps2, G, g, ak,
                                                                              # gk, demand, fixCb)
    ...
    [residual2, out_predict] = cL_pA(optx_true, test_price, G, g, 5, test_demand, C, A, opt_cb, Post_b, optx)
    ...
    print(f'The training error is {value}.')
